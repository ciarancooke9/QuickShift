...
const ChatScreen = ({ route }) => {
  ...
  const senderName = currentUser == employerId ? employerName : employeeName;
  const recipient = currentUser == employerId ? employeeId : employerId;
  const recipientName = currentUser == employerId ? employeeName : employerName;

  console.log(employeeName, employerName, recipientName, senderName);

  // Retrieve messages from Firebase
  useFocusEffect(
    useCallback(() => {
      setLoading(true);
      console.log(employeeName, employerName, recipient, sender);
